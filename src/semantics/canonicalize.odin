			...
			if lambda, ok := cbody.(^CExpr_Lambda); ok {
				for wc in where_clauses {
					...
					if !found_param {
						...
						append(
							&lambda.type_params,
							frontend.Type_Param{name = wc.type_param, constraints = constraints},
						)
					}
				}
			}
	...
	if existing_kind, ok := scope.local_kinds[id]; ok {
		if existing_kind != kind {
			...
			diagnostics.collector_add_diag(
				collector,
				diagnostics.diag_duplicate_module_name(name_str, base.Source_Span_ZERO),
			)
		}
	}
...
canonicalize_trait_method :: proc(
	...
	return CTrait_Method{name = m.name, params = params, return_type = creturn_type, span = m.span}
}

canonicalize_func_param :: proc(
	p: frontend.Func_Param,
	scope: ^Canonicalize_Scope,
	interner: ^base.Intern_Table,
	collector: ^diagnostics.Diagnostic_Collector,
) -> CFunc_Param {
...
replace_dot_receiver :: proc(expr: frontend.Expr, replacement: base.Intern_ID) -> frontend.Expr {
	...
	case ^frontend.Expr_Int,
	     ^frontend.Expr_Float,
	     ^frontend.Expr_String,
	     ^frontend.Expr_Char,
	     ^frontend.Expr_Bool,
	     ^frontend.Expr_Tag,
	     ^frontend.Expr_Nominal_Construct,
	     ^frontend.Expr_Record,
	     ^frontend.Expr_List,
	     ^frontend.Expr_Dollar_Identifier,
	     ^frontend.Expr_Lambda,
	     ^frontend.Expr_Block,
	     ^frontend.Expr_If,
	     ^frontend.Expr_Match,
	     ^frontend.Expr_BinOp,
	     ^frontend.Expr_PrefixOp,
	     ^frontend.Expr_Record_Update,
	     ^frontend.Expr_Assign,
	     ^frontend.Expr_Return,
	     ^frontend.Expr_Crash,
	     ^frontend.Expr_Todo,
	     ^frontend.Expr_Interpolated_String,
	     ^frontend.Expr_Handle,
	     ^frontend.Expr_Par,
	     ^frontend.Expr_Dot_Lambda,
	     ^frontend.Expr_For:
		return expr
	}
	return expr
...
desugar_destructure_assign :: proc(
	rec: ^frontend.Expr_Record,
	value: CExpr,
	type_ann: ^CType,
...
canonicalize_expr :: proc(
	...
	case ^frontend.Expr_Method_Call:
		...
		parallel_sugar := desugar_parallel_method(
			method_str,
			creceiver,
			args,
			e.span,
	...
	case ^frontend.Expr_Assign:
		...
		if rec, ok := e.target.(^frontend.Expr_Record);
		   ok && rec.rest == nil && !rec.is_open && len(rec.fields) > 0 {
			destructure := true
			for f in rec.fields {
			...
			if destructure {
				return desugar_destructure_assign(
					rec,
					cvalue,
					ctype_ann,
					e.span,
	...
	case ^frontend.Expr_Par:
		if e.for_var != base.Intern_ID(0) {
			...
			parallel_name := base.Canonical_Name {
				module   = base.NO_NAME,
				name     = base.intern(interner, "Parallel"),
				is_local = true,
			}
			...
			perform^ = CExpr_Perform {
				effect = parallel_name,
				op     = for_each_op,
				args   = args,
				span   = e.span,
			}
...
canonicalize_pattern :: proc(
	pat: frontend.Pattern,
	scope: ^Canonicalize_Scope,
	interner: ^base.Intern_Table,
	collector: ^diagnostics.Diagnostic_Collector,
) -> CPattern {
...
canonicalize_type :: proc(
	...
	case ^frontend.Type_Effect_Row:
		...
		for e in ty.effects {
			...
			append(
				&effects,
				CType_Effect_Entry{name = e.name, type_args = type_args, span = e.span},
			)
		}
	...
	for dt in d.derive_targets {
		...
		case "Hash":
			...
			if !generated[stub_name] {
				...
				append(
					&result,
					make_derive_method_decl(d, "hash", 1, false, scope, interner, collector),
				)
			}
		case "Ord":
			...
			if !generated[compare_name] {
				...
				append(
					&result,
					make_derive_method_decl(d, "compare", 2, false, scope, interner, collector),
				)
			}
...
make_derive_method_decl :: proc(
	...
	fn_canonical_name := base.Canonical_Name {
	...
	inner_canonical := base.Canonical_Name {
		module   = base.NO_NAME,
		name     = inner_id,
		is_local = true,
	}
	...
	method_canonical := base.Canonical_Name {
		module   = base.NO_NAME,
		name     = method_id,
		is_local = true,
	}
...
collect_type_variable_names :: proc(
	t: CType,
	seen: ^map[base.Intern_ID]bool,
	names: ^[dynamic]base.Intern_ID,
) {
	switch ty in t {
	case ^CType_Variable:
...
infer_type_params :: proc(
	...
	result := make(
		[dynamic]frontend.Type_Param,
		0,
		len(existing_type_params) + len(inferred_names),
	)
	for tp in existing_type_params {
		append(&result, tp)
	}
...
generate_record_eq_body :: proc(
	...
	if len(elements) == 0 {
		...
		true_lit^ = CExpr_Bool{value = true, span = span}
		return true_lit
	}
	
	x_rec := new(CExpr_Name)
	x_rec^ = CExpr_Name{name = base.Canonical_Name{module = base.NO_NAME, name = x_id, is_local = true}, span = span}
	...
	y_rec^ = CExpr_Name{name = base.Canonical_Name{module = base.NO_NAME, name = y_id, is_local = true}, span = span}
	...
	for i in 1 ..< len(elements) {
		...
		x_rec_i^ = CExpr_Name{name = base.Canonical_Name{module = base.NO_NAME, name = x_id, is_local = true}, span = span}
		...
		y_rec_i^ = CExpr_Name{name = base.Canonical_Name{module = base.NO_NAME, name = y_id, is_local = true}, span = span}
