...
lower_file :: proc {lower_file_cfile, lower_file_tfile}

lower_file_cfile :: proc(cfile: CFile, store: ^Type_Store) -> IR_Module {
	mod: IR_Module
...
lower_file_tfile :: proc(tfile: TFile, store: ^Type_Store) -> IR_Module {
	mod: IR_Module
...
lower_tfile :: proc(tfile: TFile, store: ^Type_Store) -> IR_Module {
	...
	env: Lower_TFile_Env = {module = &mod, store = store, interner = store.interner}
	env.pending_decls = make([dynamic]IR_Decl, 0, 8)
...
fresh_tir_name :: proc(env: ^Lower_TFile_Env) -> Intern_ID {
	name := fmt.tprintf("_tir_{}", env.fresh_counter)
	env.fresh_counter += 1
	return intern(env.interner, name)
}

lower_tdecl_const :: proc(d: TDecl_Const, env: ^Lower_TFile_Env) -> IR_Decl {
	#partial switch body_expr in d.body {
	case ^TExpr_Lambda:
		return lower_tlambda_as_decl(body_expr, d.name, d.is_effectful, d.span, env)
	case:
	...
	decl^ = IR_Decl_Const{
		name = d.name,
		type = d.type_,
		value = lower_texpr(d.body, env),
		span = d.span,
	}
	return IR_Decl(decl)
}

lower_tlambda_as_decl :: proc(e: ^TExpr_Lambda, name: Canonical_Name, is_effectful: bool, span: Source_Span, env: ^Lower_TFile_Env) -> IR_Decl {
	params := make([dynamic]IR_Param, 0, len(e.params))
	for p in e.params {
		append(&params, IR_Param{name = p.name, type = p.type_})
	}
...
lower_tdecl_effect :: proc(d: TDecl_Effect, env: ^Lower_TFile_Env) -> IR_Decl {
	ops := make([dynamic]IR_Effect_Op, 0, len(d.operations))
	for op in d.operations {
		append(&ops, lower_teffect_op(op, env))
	}
...
lower_teffect_def :: proc(d: TDecl_Effect, env: ^Lower_TFile_Env) -> IR_Effect_Def {
	ops := make([dynamic]IR_Effect_Op, 0, len(d.operations))
	for op in d.operations {
		append(&ops, lower_teffect_op(op, env))
	}
...
lower_teffect_op :: proc(op: TEffect_Op, env: ^Lower_TFile_Env) -> IR_Effect_Op {
	params := make([dynamic]IR_Param, 0, len(op.params))
	for p in op.params {
		append(&params, IR_Param{name = p.name, type = p.type_})
	}
...
lower_texpr :: proc(expr: TExpr, env: ^Lower_TFile_Env) -> IR_Expr {
	switch e in expr {
	case ^TExpr_Int:
		lit := new(IR_Literal_Int)
		lit^ = IR_Literal_Int{value = e.value, type = e.type_, span = e.span}
		return IR_Expr(lit)
	...
	case ^TExpr_String:
		lit := new(IR_Literal_String)
		lit^ = IR_Literal_String{value = e.value, type = e.type_, span = e.span}
		append(&env.module.string_table, String_Table_Entry{id = fresh_tir_name(env), value = e.value})
		return IR_Expr(lit)

	case ^TExpr_Bool:
		lit := new(IR_Literal_Bool)
		lit^ = IR_Literal_Bool{value = e.value, type = e.type_, span = e.span}
		return IR_Expr(lit)
...
lower_tcall :: proc(e: ^TExpr_Call, env: ^Lower_TFile_Env) -> IR_Expr {
	#partial switch c in e.callee {
	case ^TExpr_Name:
		callee_name := c.name
		ir_args := make([dynamic]IR_Expr, 0, len(e.args))
...
lower_tmethod_call :: proc(e: ^TExpr_Method_Call, env: ^Lower_TFile_Env) -> IR_Expr {
	receiver := lower_texpr(e.receiver, env)
...
lower_tlambda :: proc(e: ^TExpr_Lambda, env: ^Lower_TFile_Env) -> IR_Expr {
	fn_name := Canonical_Name{module = NO_NAME, name = fresh_tir_name(env), is_local = true}

	param_types := make([dynamic]IR_Param, 0, len(e.params))
	for p in e.params {
		append(&param_types, IR_Param{name = p.name, type = p.type_})
	}
...
lower_tblock :: proc(e: ^TExpr_Block, env: ^Lower_TFile_Env) -> IR_Expr {
	if len(e.statements) == 0 {
		block := new(IR_Block)
		block^ = IR_Block{statements = make([dynamic]IR_Expr, 0), type = e.type_, span = e.span}
		return IR_Expr(block)
	}

	result := lower_texpr(e.statements[len(e.statements)-1], env)
	...
	for i := len(e.statements) - 2; i >= 0; i -= 1 {
		...
		stmts := make([dynamic]IR_Expr, 2)
		stmts[0] = lower_texpr(e.statements[i], env)
		stmts[1] = result
		block := new(IR_Block)
...
lower_tif :: proc(e: ^TExpr_If, env: ^Lower_TFile_Env) -> IR_Expr {
	cond := lower_texpr(e.condition, env)
	then_br := lower_texpr(e.then_branch, env)
	else_br := lower_texpr(e.else_branch, env)
...
lower_tmatch :: proc(e: ^TExpr_Match, env: ^Lower_TFile_Env) -> IR_Expr {
	scrutinee := lower_texpr(e.scrutinee, env)

	arms := make([dynamic]IR_Match_Arm, 0, len(e.arms))
	for arm in e.arms {
		append(&arms, IR_Match_Arm{
			pattern = lower_tpattern(arm.pattern, env),
			body = lower_texpr(arm.body, env),
		})
	}
...
lower_tpattern :: proc(pat: TPattern, env: ^Lower_TFile_Env) -> IR_Pattern {
	switch p in pat {
	case ^TPattern_Tag:
		payload_ids := make([dynamic]Intern_ID, 0, len(p.payload))
		for sub in p.payload {
			...
			case ^TPattern_Identifier:
				append(&payload_ids, s.name)
			case:
				append(&payload_ids, fresh_tir_name(env))
			}
		}
		ir_pat := new(IR_Pat_Tag)
		ir_pat^ = IR_Pat_Tag{name = p.name.name, payload = payload_ids}
	...
	case ^TPattern_Int, ^TPattern_String, ^TPattern_Bool, ^TPattern_List, ^TPattern_Destructure:
		ir_pat := new(IR_Pat_Wildcard)
		ir_pat^ = IR_Pat_Wildcard{}
		return IR_Pattern(ir_pat)
	}
...
lower_tbinop :: proc(e: ^TExpr_BinOp, env: ^Lower_TFile_Env) -> IR_Expr {
	left := lower_texpr(e.left, env)
	right := lower_texpr(e.right, env)
...
lower_tprefixop :: proc(e: ^TExpr_PrefixOp, env: ^Lower_TFile_Env) -> IR_Expr {
	operand := lower_texpr(e.operand, env)

	#partial switch e.op {
	case .Kw_Not:
		bool_type := IR_Type{.I32, e.type_.type_id}
		false_lit := make_ir_lit_bool(false, bool_type, e.span)
		binop := new(IR_BinOp)
		binop^ = IR_BinOp{op = .Eq_Eq, left = operand, right = false_lit, type = bool_type, span = e.span}
		return IR_Expr(binop)
	case .Minus:
		zero_lit := make_ir_lit_int(0, e.type_, e.span)
		binop := new(IR_BinOp)
		binop^ = IR_BinOp{op = .Minus, left = zero_lit, right = operand, type = e.type_, span = e.span}
		return IR_Expr(binop)
	case:
		return operand
	}
...
lower_ttag :: proc(e: ^TExpr_Tag, env: ^Lower_TFile_Env) -> IR_Expr {
	if is_declared_newtype(env.store, e.name.name) && len(e.payload) == 1 {
		return lower_texpr(e.payload[0], env)
	}
...
lower_trecord :: proc(e: ^TExpr_Record, env: ^Lower_TFile_Env) -> IR_Expr {
	fields := make([dynamic]IR_Record_Field, 0, len(e.fields))
	for f in e.fields {
		append(&fields, IR_Record_Field{name = f.name, value = lower_texpr(f.value, env)})
	}
...
lower_tfield_access :: proc(e: ^TExpr_Field_Access, env: ^Lower_TFile_Env) -> IR_Expr {
	record := lower_texpr(e.record, env)
...
lower_trecord_update :: proc(e: ^TExpr_Record_Update, env: ^Lower_TFile_Env) -> IR_Expr {
	rest := lower_texpr(e.rest, env)

	fields := make([dynamic]IR_Record_Field, 0, len(e.updates))
	for u in e.updates {
		append(&fields, IR_Record_Field{name = u.name, value = lower_texpr(u.value, env)})
	}
...
lower_tinterpolate :: proc(e: ^TExpr_Interpolate, env: ^Lower_TFile_Env) -> IR_Expr {
	if len(e.parts) == 0 {
		lit := new(IR_Literal_String)
		lit^ = IR_Literal_String{value = "", type = e.type_, span = e.span}
		return IR_Expr(lit)
...
lower_thandle :: proc(e: ^TExpr_Handle, env: ^Lower_TFile_Env) -> IR_Expr {
	body := lower_texpr(e.body, env)
...
lower_tlist :: proc(e: ^TExpr_List, env: ^Lower_TFile_Env) -> IR_Expr {
	ir_type := e.type_
