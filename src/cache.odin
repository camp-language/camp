...
build_manifest :: proc(mi: ^Module_Info, interner: ^Intern_Table) -> Module_Manifest {
	manifest: Module_Manifest
	manifest.content_hash = mi.content_hash
	manifest.module_name = intern_get(interner, mi.name)
	...
	for imp in mi.imports {
		...
		mi_imp.module = intern_get(interner, imp.module)
		mi_imp.exposing = make([dynamic]string, 0, len(imp.exposing))
		for exp in imp.exposing {
			append(&mi_imp.exposing, intern_get(interner, exp))
		}
		if imp.alias != NO_NAME {
			mi_imp.alias = intern_get(interner, imp.alias)
		}
...
cache_write_manifest :: proc(mi: ^Module_Info, interner: ^Intern_Table) -> bool {
