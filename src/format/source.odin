...
is_separator :: proc(kind: base.Token_Kind) -> bool {
	#partial switch kind {
	case .Comma, .Pipe:
		return true
...
is_open_delim :: proc(kind: base.Token_Kind) -> bool {
	return kind == .LParen || kind == .LBrack || kind == .LBrace
}

is_close_delim :: proc(kind: base.Token_Kind) -> bool {
	return kind == .RParen || kind == .RBrack || kind == .RBrace
}

analyze_source :: proc(source: string, tokens: []base.Token) -> Format_Source_Info {
...
separator_analysis :: proc(source: string, tokens: []base.Token, info: ^Format_Source_Info) {
	depth := 0
...
source_text_analysis :: proc(source: string, tokens: []base.Token, info: ^Format_Source_Info) {
