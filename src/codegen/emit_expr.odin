...
collect_pattern_locals :: proc(pattern: ir.IR_Pattern, locals: ^map[base.Intern_ID]base.IR_Type) {
	...
	case ^ir.IR_Pat_Var:
		locals^[p.name] = base.IR_Type{wasm_type = .I32, type_id = base.Type_Var_ID(0)}
	case ^ir.IR_Pat_Record:
		for f in p.fields {
			locals^[f.binding] = base.IR_Type{wasm_type = .I32, type_id = base.Type_Var_ID(0)}
		}
...
emit_expr :: proc(expr: ir.IR_Expr, buf: ^[dynamic]u8, env: ^Codegen_Env, runtime_indices: []int) {
	...
	case ^ir.IR_Call:
		...
		if e.callee.module != base.NO_NAME {
			...
			if module_str == "Str" {
				if name_str == "length" && len(e.args) == 1 {
					...
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Str_Len])}, buf)
				...
				if name_str == "concat" && len(e.args) == 2 {
					emit_expr(e.args[0], buf, env, runtime_indices)
					emit_expr(e.args[1], buf, env, runtime_indices)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Str_Concat])}, buf)
					break
				}
			}
			...
			if module_str == "List" {
				if name_str == "length" && len(e.args) == 1 {
					emit_expr(e.args[0], buf, env, runtime_indices)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.List_Len])}, buf)
				...
				if name_str == "get" && len(e.args) == 2 {
					emit_expr(e.args[0], buf, env, runtime_indices)
					emit_expr(e.args[1], buf, env, runtime_indices)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.List_Get])}, buf)
					break
				}
				if name_str == "push" && len(e.args) == 2 {
					emit_expr(e.args[0], buf, env, runtime_indices)
					emit_expr(e.args[1], buf, env, runtime_indices)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.List_Push])}, buf)
					break
				}
				if name_str == "alloc" && len(e.args) == 0 {
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.List_Alloc])}, buf)
					break
				}
			}
		}
		...
		if e.callee.module == base.NO_NAME {
			...
			if name_str == "Bool_to_str" && len(e.args) == 1 {
				emit_expr(e.args[0], buf, env, runtime_indices)
				emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Bool_To_Str])}, buf)
				break
			}
			...
			if effect_str == "Async!" || effect_str == "Spawn!" {
				...
				if op_str == "spawn!" {
					...
					if len(e.args) >= 1 {
						...
						emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Sched_Spawn])}, buf)
					} else {
						emit_instruction(Wasm_I32_Const{value = 0}, buf)
					}
				} else if op_str == "join!" {
					...
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Sched_Join])}, buf)
					...
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Sched_Yield])}, buf)
					...
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Sched_Cancel])}, buf)
				} else {
					emit_instruction(Wasm_Unreachable{}, buf)
				}
				...
				if op_str == "sleep!" {
					...
					emit_instruction(Wasm_I32_Const{value = 0}, buf)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Sched_Timer_Insert])}, buf)
				} else {
					emit_instruction(Wasm_Unreachable{}, buf)
				}
				...
				if op_str == "read!" || op_str == "readln!" {
					...
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Sched_Block_IO])}, buf)
				...
				if op_str == "map!" && len(e.args) >= 2 {
					...
					emit_instruction(Wasm_I32_Load{align = 2, offset = u32(CAMP_TAG_FIELDS_OFFSET + 8)}, buf)
					emit_instruction(Wasm_I32_Const{value = 0}, buf)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Parallel_Map])}, buf)
					...
					emit_instruction(Wasm_I32_Const{value = 0}, buf)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Parallel_Reduce])}, buf)
					...
					emit_instruction(Wasm_Local_Get{index = items_local}, buf)
					emit_instruction(Wasm_I32_Load{align = 2, offset = u32(CAMP_TAG_FIELDS_OFFSET + 8)}, buf)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Parallel_Any])}, buf)
					...
					emit_instruction(Wasm_I32_Load{align = 2, offset = u32(CAMP_TAG_FIELDS_OFFSET + 8)}, buf)
					emit_instruction(Wasm_I32_Const{value = 0}, buf)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Parallel_All])}, buf)
					...
					emit_instruction(Wasm_I32_Load{align = 2, offset = u32(CAMP_TAG_FIELDS_OFFSET + 8)}, buf)
					emit_instruction(Wasm_I32_Const{value = 0}, buf)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Parallel_Filter])}, buf)
					...
					emit_instruction(Wasm_I32_Load{align = 2, offset = u32(CAMP_TAG_FIELDS_OFFSET + 8)}, buf)
					emit_instruction(Wasm_I32_Const{value = 0}, buf)
					emit_instruction(Wasm_Call{index = u32(runtime_indices[Runtime_Func.Parallel_For_Each])}, buf)
				} else {
					emit_instruction(Wasm_Unreachable{}, buf)
				}
			} else {
	...
	case ^ir.IR_Wait:
		...
		if e.width == .B8 {
			emit_instruction(Wasm_Atomic_Mem{op = .Wait64, width = .I64, align = 3, offset = u32(e.offset)}, buf)
		} else {
			emit_instruction(Wasm_Atomic_Mem{op = .Wait32, width = .I32, align = 2, offset = u32(e.offset)}, buf)
		}
	case ^ir.IR_Notify:
		emit_expr(e.base, buf, env, runtime_indices)
		emit_expr(e.count, buf, env, runtime_indices)
		emit_instruction(Wasm_Atomic_Mem{op = .Notify, width = .I32, align = 2, offset = u32(e.offset)}, buf)
...
ir_operand_wasm_type :: proc(expr: ir.IR_Expr) -> base.IR_Wasm_Type {
	...
	#partial switch e in expr {
	case ^ir.IR_Literal_Int: return e.type.wasm_type
	case ^ir.IR_Literal_Float: return e.type.wasm_type
	case ^ir.IR_Literal_Bool: return .I32
	case ^ir.IR_Literal_String: return .I32
	case ^ir.IR_Var: return e.type.wasm_type
	case ^ir.IR_BinOp: return e.type.wasm_type
	case ^ir.IR_Assign: return e.type.wasm_type
	case ^ir.IR_Loop: return e.type.wasm_type
	case ^ir.IR_Call: return e.type.wasm_type

	case ^ir.IR_If: return e.type.wasm_type
	case ^ir.IR_Closure_Call: return e.type.wasm_type
	case ^ir.IR_Resume: return e.type.wasm_type
	case ^ir.IR_Field_Access: return e.type.wasm_type
	case ^ir.IR_Construct_Tag: return .I32
	case ^ir.IR_Construct_Record: return .I32
	case ^ir.IR_Closure: return .I32
	case ^ir.IR_Atomic_Load: return .I32
...
emit_atomic_load :: proc(width: ir.Atomic_Width, offset: u32, buf: ^[dynamic]u8) {
	...
	case .B1:
		emit_instruction(Wasm_Atomic_Mem{op = .Load, width = .I32_8, align = 0, offset = offset}, buf)
	case .B2:
		emit_instruction(Wasm_Atomic_Mem{op = .Load, width = .I32_16, align = 1, offset = offset}, buf)
	case .B4:
		emit_instruction(Wasm_Atomic_Mem{op = .Load, width = .I32, align = 2, offset = offset}, buf)
	case .B8:
		emit_instruction(Wasm_Atomic_Mem{op = .Load, width = .I64, align = 3, offset = offset}, buf)
	}
}
...
emit_atomic_store :: proc(width: ir.Atomic_Width, offset: u32, buf: ^[dynamic]u8) {
	#partial switch width {
	case .B1:
		emit_instruction(Wasm_Atomic_Mem{op = .Store, width = .I32_8, align = 0, offset = offset}, buf)
	case .B2:
		emit_instruction(Wasm_Atomic_Mem{op = .Store, width = .I32_16, align = 1, offset = offset}, buf)
	case .B4:
		emit_instruction(Wasm_Atomic_Mem{op = .Store, width = .I32, align = 2, offset = offset}, buf)
	case .B8:
		emit_instruction(Wasm_Atomic_Mem{op = .Store, width = .I64, align = 3, offset = offset}, buf)
	}
}
...
emit_atomic_rmw :: proc(op: ir.Atomic_Op, width: ir.Atomic_Width, offset: u32, buf: ^[dynamic]u8) {
	...
	switch op {
	case .Add:     wasm_op = .RMW_Add
	case .Sub:     wasm_op = .RMW_Sub
	case .And:     wasm_op = .RMW_And
	case .Or:      wasm_op = .RMW_Or
	case .Xor:     wasm_op = .RMW_Xor
	case .Xchg:    wasm_op = .RMW_Xchg
	case .CmpXchg: wasm_op = .RMW_CmpXchg
	}
	wasm_width := Wasm_Atomic_Width(0)
	#partial switch width {
	case .B1: wasm_width = .I32_8
	case .B2: wasm_width = .I32_16
	case .B4: wasm_width = .I32
	case .B8: wasm_width = .I64
	}
	align := u32(0)
	#partial switch width {
	case .B1: align = 0
	...
	emit_instruction(Wasm_Atomic_Mem{op = wasm_op, width = wasm_width, align = align, offset = offset}, buf)
}
