...
parser_expect :: proc(p: ^Parser, kind: base.Token_Kind) -> base.Token {
	...
	diagnostics.collector_add_diag(
		p.collector,
		diagnostics.diag_expected_token(kind, p.current, expected_span),
	)
	parser_advance(p)
		...
		for p.current.kind != .RBrace && p.current.kind != .Eof {
			...
			case ^Type_Function:
				for pt in t.params {
					...
					append(
						&op_params,
						Func_Param{name = 0, type_ann = param_type_ann, span = op_tok.span},
					)
				}
...
parser_parse_tag_or_call :: proc(p: ^Parser) -> Expr {
	...
	if p.current.kind == .LParen {
		...
		if len(tag.payload) == 0 {
			diagnostics.collector_add_diag(
				p.collector,
				diagnostics.diag_empty_tag_parens(name_tok.text, tag.span),
			)
		}
		parser_expect(p, .RParen)
	}
...
parser_parse_lambda :: proc(p: ^Parser) -> Expr {
	...
	for p.current.kind != .Pipe && p.current.kind != .Eof {
		...
		if p.current.kind == .Identifier ||
		   p.current.kind == .Upper_Id ||
		   p.current.kind == .Kw_Self {
			name_tok := parser_advance(p)
...
parser_parse_expr_or_decl :: proc(p: ^Parser) -> Expr {
	if p.current.kind == .Kw_Expect {
		...
		id^ = Expr_Identifier {
			name = base.intern(p.intern, "expect"),
			span = p.current.span,
		}
		args := make([dynamic]Expr, 0, 4)
		append(&args, cond)
	...
	if p.current.kind == .Dollar {
		...
		if p.current.kind == .Eq || p.current.kind == .Colon {
			...
			id_expr^ = Expr_Dollar_Identifier {
				name = name_id,
				span = start,
			}
...
parser_parse_if :: proc(p: ^Parser) -> Expr {
	...
	if p.current.kind == .LBrace {
		...
		diagnostics.collector_add_diag(
			p.collector,
			diagnostics.diag_if_requires_braces(p.current.span),
		)
...
parser_parse_handle :: proc(p: ^Parser) -> Expr {
	...
	for p.current.kind != .RBrace && p.current.kind != .Eof {
		...
		append(
			&arms,
			Handler_Arm{op = op_id, params = params, body = arm_body, span = op_tok.span},
		)
		if p.current.kind == .Comma {
			parser_advance(p)
		}
			...
			if p.current.kind == .Identifier {
				...
				if next_tok.kind == .Colon {
					...
					ident^ = Expr_Identifier {
						name = base.intern(p.intern, name_tok.text),
						span = name_tok.span,
					}
				...
				diagnostics.collector_add_diag(
					p.collector,
					diagnostics.diag_par_entry_must_be_named(span),
				)
				expr := parser_parse_expr(p)
				append(&e.expressions, expr)
			}
...
parser_parse_pattern :: proc(p: ^Parser) -> Pattern {
	...
	case .Upper_Id:
		...
		pat^ = Pattern_Tag {
			name    = name_id,
			payload = make([dynamic]Pattern, 0, 2),
			span    = name_tok.span,
		}
...
parser_parse_type :: proc(p: ^Parser) -> ^Type {
	...
	case .Int_Literal,
		...
		diagnostics.collector_add_diag(
			p.collector,
			diagnostics.diag_expected_type(p.current, p.current.span),
		)
		parser_advance(p)
		v := new(Type_Variable)
		v^ = Type_Variable {
...
parser_parse_trait_decl :: proc(p: ^Parser, is_pub: bool) -> Decl {
	...
	for p.current.kind != .RBrace && p.current.kind != .Eof {
		...
		append(
			&methods,
			Trait_Method{name = m_name_id, return_type = return_type, span = m_name_tok.span},
		)
	...
	for i < len(inner_text) {
		if inner_text[i] == '\\' && i + 1 < len(inner_text) && inner_text[i + 1] == '$' {
			...
			if brace_depth != 0 {
				diagnostics.collector_add_diag(
					p.collector,
					diagnostics.diag_unterminated_interpolation(tok),
				)
			...
			if has_newline {
				diagnostics.collector_add_diag(
					p.collector,
					diagnostics.diag_multiline_interpolation(tok),
				)
			}
...
parse_interpolation_expr :: proc(text: string, span: base.Source_Span, p: ^Parser) -> Expr {
	...
	if sub.current.kind != .Eof {
		diagnostics.collector_add_diag(
			p.collector,
			diagnostics.diag_unexpected_tokens_after_interpolation(tok = sub.current),
		)
	}
