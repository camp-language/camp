...
CPS_Env :: struct {
	...
	module:        ^IR_Module,
	effectful_fns: map[base.Canonical_Name]bool,
	fresh_state:   base.Fresh_State,
}


cps_make_continuation :: proc(
	body: IR_Expr,
	param_name: base.Intern_ID,
	return_type: base.IR_Type,
	k_name: base.Intern_ID,
	env: ^CPS_Env,
) -> base.Canonical_Name {
	cont_name := base.Canonical_Name {
		module   = base.NO_NAME,
...
cps_transform :: proc(mod: ^IR_Module, interner: ^base.Intern_Table) -> IR_Module {
	result: IR_Module
...
cps_transform_decl :: proc(decl: IR_Decl, env: ^CPS_Env) -> IR_Decl {
	...
	case ^IR_Decl_Fn:
		...
		new_fn.return_type = base.IR_Type {
			wasm_type = .Void,
			type_id   = base.Type_Var_ID(0),
		}
		...
		result_name := base.fresh_id(&env.fresh_state, "_result")
...
cps_transform_expr :: proc(expr: IR_Expr, k_name: base.Intern_ID, env: ^CPS_Env) -> IR_Expr {
	...
	case ^IR_Return:
		...
		ret_type := base.IR_Type {
			wasm_type = ir_expr_wasm_type(e.value),
			type_id   = 0,
		}
	...
	case ^IR_Let:
		...
		case ^IR_Call:
			if v.callee in env.effectful_fns {
				result_name := base.fresh_id(&env.fresh_state, "_r")
				result_type := v.type

				cont_name := cps_make_continuation(e.body, result_name, result_type, k_name, env)
				...
				cont_var^ = IR_Var {
					name = cont_name.name,
					type = base.IR_Type{wasm_type = .Funcref, type_id = base.Type_Var_ID(0)},
					span = e.span,
				}
	...
	case ^IR_Match:
		...
		for arm in e.arms {
			append(
				...
				IR_Match_Arm {
					pattern = arm.pattern,
					guard = cps_transform_expr(arm.guard, k_name, env) if arm.guard != nil else nil,
	...
	case ^IR_Construct_Record:
		...
		for f in e.fields {
			append(
				&new_fields,
				IR_Record_Field{name = f.name, value = cps_transform_expr(f.value, k_name, env)},
			)
		}
	...
	case ^IR_Crash:
		...
		new_crash^ = IR_Crash {
			message = cps_transform_expr(e.message, k_name, env),
			span    = e.span,
		}
