...
Add a tree-sitter grammar for Camp, hosted in-repo under `tree-sitter/`. Provides syntax highlighting, scope tracking, and symbol tagging for editors. Corpus tests prevent parser/grammar drift.
...
- **Lightweight**: CI runs `tree-sitter test` + `odin test src` on every commit. Corpus tests break when grammar falls behind.
- **Medium**: `tree-sitter-validate` parses all e2e `.camp` files and fails on any `ERROR` node.
- **Heavy** (future): Snapshot-based drift detection. Regenerate all corpus expected outputs from real `.camp` files and commit them.
